...
#### Plotting Composite Score

# Sort the composite_score by 'composite_score' column in descending order
sorted_composite_score <- composite_score %>% 
  arrange(desc(composite_score))

# Show the top 5 highest scores
highest_scores <- head(sorted_composite_score, 5)
highest_scores

######
# Fig. 12: Final Score metrics

# Use the formattable package for a nice table
library(formattable)

# Show the top N scores
top_n_scores <- head(sorted_composite_score, 20)

cat("Composite Score - Field Testing Data\n")

# Create a color-coded table
formattable(top_n_scores, list(
  composite_score = color_tile("white", "steelblue")
))

formattable(sorted_composite_score, list(
  composite_score = color_tile("white", "steelblue")
))
