...
count_SCL <- table(ex_ht_All_df$SCL)
print(count_SCL)

### Fig. 12: Plotting heatmap of correlations ####
...
#### Plotting correlation pltos
install.packages("corrplot")
library(corrplot)
cor_matrix <- cor(ex_ht_All_df_rm_df)

# Use corrplot to create the heatmap
...
#  geom_text(aes(label = round(value, 2)), size = 3) +
  ...
  labs(title = paste0("Correlation Heatmap N = ",size[1]), x = "", y = "")+
...
#### Show the correlations with the importance scores?

### Correlation iwth Rh60
cor_values <- cor_matrix[predictors,'rh60']
names <- names(cor_values)
barplot(cor_values, names.arg = names, las = 2, main = "Correlation with rh60", cex.names = 0.7, col = "lightblue")
dotchart(cor_values, labels = names, main = "Correlation with rh60", cex = 0.7)

### Correlation with importance score
aa = varImp(model_2019[[6]]$model)
...
data <- data.frame(
  ...
  Importance = aa$importance$Overall,
...
library(ggplot2)
#### Fig. Scatter Plot vs Importance score Final #####
# Scatter plot
plot <- ggplot(data, aes(x = Correlation, y = Importance)) +
  geom_point(color = "yellow", size = 3, alpha = 0.7) +
  geom_text(aes(label = Variable), nudge_y = 1.5, vjust = "inward", size = 3.5) +
  geom_vline(xintercept = 0,color = "black", linetype = "solid", size = 0.1) + # Line at x=0
  #geom_vline(yintercept = 0,color = "black", linetype = "solid", size = 0.5) + # Line at x=0
  labs(title = "Scatter Plot of Importance vs. Correlation for Rh60", 
       x = "Correlation", y = "Importance") +
  theme_minimal() +
  theme(
    legend.position = "none", # Hides the color legend for importance
    panel.border = element_rect(color = "black", fill = NA, size = 0.5) # Adds a solid border to the panel
  )+
  theme(plot.title = element_text(hjust = 0.5))
plot
...
library(gridExtra)
#saveRDS(results_all, "Models, summarymetrics_predictions_2019_cropped_night_time.rds")
